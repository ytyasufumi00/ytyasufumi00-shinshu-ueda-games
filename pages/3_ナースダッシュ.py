...
<script>
    ...
    const nurseQuizzes = [
       ...
       // 【糖尿病・代謝内分泌】
        { q: "糖尿病の3大合併症「しめじ」の「し」は何？", options: ["神経障害", "心筋梗塞", "歯周病", "しらたき"], ans: "神経障害" },
        { q: "糖尿病の指標となるHbA1cは、過去どれくらいの期間の平均血糖値を反映する？", options: ["過去1〜2ヶ月", "過去1週間", "過去半年", "前世の記憶"], ans: "過去1〜2ヶ月" },
        { q: "インスリン注射を行う際、毎回全く同じ部位に注射し続けるとどうなる？", options: ["硬結（皮膚が硬く盛り上がる）", "筋肉がムキムキになる", "血糖値が下がりすぎる", "ツボが押されて元気になる"], ans: "硬結（皮膚が硬く盛り上がる）" },
        { q: "甲状腺機能亢進症（バセドウ病）の代表的な症状はどれ？", options: ["頻脈・体重減少・発汗", "徐脈・体重増加・寒がり", "激しい関節痛", "圧倒的なカリスマ性"], ans: "頻脈・体重減少・発汗" },
        { q: "クッシング症候群に特徴的な身体的所見はどれ？", options: ["満月様顔貌（ムーンフェイス）", "著しいやせ", "眼球突出", "後光が射す"], ans: "満月様顔貌（ムーンフェイス）" },
        { q: "低血糖の初期症状（交感神経症状）として見られるものはどれ？", options: ["冷や汗・手の震え・動悸", "深い昏睡", "幻覚・妄想", "謎の万能感"], ans: "冷や汗・手の震え・動悸" },
        { q: "尿崩症で見られる特徴的な症状はどれ？", options: ["多尿・口渇・多飲", "無尿", "血尿", "恋の病"], ans: "多尿・口渇・多飲" },

        // 【膠原病】
        { q: "全身性エリテマトーデス（SLE）に特徴的な顔の皮疹を何という？", options: ["蝶形紅斑（バタフライ・ラッシュ）", "ヘリオトロープ疹", "結節性紅斑", "歌舞伎の隈取"], ans: "蝶形紅斑（バタフライ・ラッシュ）" },
        { q: "関節リウマチで最もよく見られる初期症状はどれ？", options: ["朝の関節のこわばり", "足の親指の付け根の激痛", "腰痛", "全身から覇気が出る"], ans: "朝の関節のこわばり" },
        { q: "膠原病の治療でよく使われるステロイド薬。長期内服時の副作用として注意すべきなのは？", options: ["骨粗鬆症・感染症", "聴力低下", "喘息", "悟りを開く"], ans: "骨粗鬆症・感染症" },
        { q: "シェーグレン症候群で特に見られる症状はどれ？", options: ["ドライアイ・ドライマウス", "激しい下痢", "呼吸困難", "涙腺の崩壊による滝涙"], ans: "ドライアイ・ドライマウス" },
        { q: "皮膚筋炎において、上眼瞼（まぶた）に見られる浮腫性の紅斑を何という？", options: ["ヘリオトロープ疹", "ゴットロン徴候", "蝶形紅斑", "ものもらい"], ans: "ヘリオトロープ疹" },
        { q: "痛風の激痛発作が最も起こりやすい部位はどこ？", options: ["足の親指の付け根", "膝関節", "肩関節", "ハート"], ans: "足の親指の付け根" },

        // 【胸部外科】
        { q: "気胸（肺に穴が開き空気が漏れる病気）の治療で行われる処置はどれ？", options: ["胸腔ドレナージ", "胃瘻造設", "心臓マッサージ", "気合で穴を塞ぐ"], ans: "胸腔ドレナージ" },
        { q: "胸腔ドレーンが入っている患者。ドレーンの水封部の水面が呼吸に合わせて上下する現象を何という？", options: ["呼吸性移動", "エアリーク", "バブリング", "シンクロニシティ"], ans: "呼吸性移動" },
        { q: "肺がんの手術後、肺を広げて無気肺や肺炎を予防するために患者に促すことは？", options: ["早期離床と深呼吸・咳嗽（せき）", "1週間の絶対安静", "水分を制限する", "全集中の呼吸"], ans: "早期離床と深呼吸・咳嗽（せき）" },
        { q: "大動脈解離の分類（スタンフォード分類）で、上行大動脈に解離がある緊急手術適応の重症タイプはどっち？", options: ["A型", "B型", "C型", "O型"], ans: "A型" },
        { q: "心臓の冠動脈バイパス術（CABG）でバイパスの血管としてよく使われるのはどこ？", options: ["内胸動脈や大伏在静脈", "頸動脈", "大動脈", "ヘビの抜け殻"], ans: "内胸動脈や大伏在静脈" },
        { q: "胸腔ドレーンの排液バッグを置く位置として正しいのはどれ？", options: ["患者の胸の高さより下", "患者の胸より高い位置", "ベッドの上", "天井から吊るす"], ans: "患者の胸の高さより下" },

        // 【腹部外科】
        { q: "腹部の手術後、腸の動きが回復する前に飲食を再開すると起こりやすい合併症は？", options: ["イレウス（腸閉塞）", "胃潰瘍", "虫垂炎", "食神の降臨"], ans: "イレウス（腸閉塞）" },
        { q: "虫垂炎（盲腸）の診断で有名な、右下腹部の圧痛点を何という？", options: ["マックバーニー点", "ブルンベルグ徴候", "マーフィー徴候", "スウィートスポット"], ans: "マックバーニー点" },
        { q: "胃切除術後の患者に見られる「ダンピング症候群」。食事の工夫として正しいのは？", options: ["少量頻回食にし、ゆっくりよく噛んで食べる", "1日1回ドカ食いする", "流動食のみにする", "ラーメン二郎の完食"], ans: "少量頻回食にし、ゆっくりよく噛んで食べる" },
        { q: "胆石症で痛み（胆道疝痛）が最も起きやすいのはどんな時？", options: ["油っこい食事を食べた後", "空腹時", "朝起きた直後", "上司に怒られた時"], ans: "油っこい食事を食べた後" },
        { q: "大腸がんの手術等で、便を外に出すためにお腹に作られる「人工肛門」のことを何と呼ぶ？", options: ["ストーマ", "カテーテル", "ドレーン", "シークレットベース"], ans: "ストーマ" },
        { q: "腹腔鏡手術の際、お腹の中を見やすく（スペースを作る）ために注入されるガスは何？", options: ["二酸化炭素", "酸素", "ヘリウム", "笑気ガス"], ans: "二酸化炭素" },

        // 【一般内科】
        { q: "高血圧の患者に対する生活指導で、最も重要なのはどれ？", options: ["塩分制限（1日6g未満）", "糖質制限", "タンパク質制限", "滝行で精神を鍛える"], ans: "塩分制限（1日6g未満）" },
        { q: "貧血の中で最も頻度が高く、鉄分不足によって起こるものは？", options: ["鉄欠乏性貧血", "巨赤芽球性貧血", "再生不良性貧血", "金欠性貧血"], ans: "鉄欠乏性貧血" },
        { q: "インフルエンザなどの主な感染経路はどれ？", options: ["飛沫感染・接触感染", "空気感染のみ", "血液媒介感染", "Bluetooth接続"], ans: "飛沫感染・接触感染" },
        { q: "マイコプラズマ肺炎の治療によく使われる抗菌薬（抗生物質）の系統は？", options: ["マクロライド系", "ペニシリン系", "セフェム系", "魔法の薬"], ans: "マクロライド系" },
        { q: "逆流性食道炎（GERD）の患者への生活指導で適切なのはどれ？", options: ["食後すぐに横にならない", "食後は右を下にして寝る", "油物をたくさん食べる", "食後は全力疾走する"], ans: "食後すぐに横にならない" },
        { q: "健康診断で「肝機能障害」を疑う指標となる代表的な血液データはどれ？", options: ["AST(GOT)・ALT(GPT)", "BUN・クレアチニン", "HbA1c", "今年の運勢"], ans: "AST(GOT)・ALT(GPT)" },
        { q: "高齢者に多い「誤嚥性肺炎」の主な原因はどれ？", options: ["唾液や食べ物が誤って気管に入ること", "ウイルスの空気感染", "アレルギー反応", "むせび泣き"], ans: "唾液や食べ物が誤って気管に入ること" },

        // 【泌尿器科】
        { q: "高齢男性に多い「前立腺肥大症」の代表的な症状はどれ？", options: ["頻尿・排尿困難・残尿感", "大量の血尿", "激しい腰痛", "トイレでの悟り"], ans: "頻尿・排尿困難・残尿感" },
        { q: "膀胱炎に罹りやすいのは男女どちら？またその理由は？", options: ["女性（尿道が短いため）", "男性（尿道が長いため）", "男性（前立腺があるため）", "女性（我慢強いため）"], ans: "女性（尿道が短いため）" },
        { q: "尿路結石（腎結石・尿管結石）で最も多い結石の成分はどれ？", options: ["シュウ酸カルシウム", "尿酸", "シスチン", "愛の結晶"], ans: "シュウ酸カルシウム" },
        { q: "尿管結石の発作時に見られる、背中から腰にかけてののたうち回るような激しい痛みを何という？", options: ["疝痛（せんつう）発作", "放散痛", "関連痛", "ギックリ腰"], ans: "疝痛（せんつう）発作" },
        { q: "男性特有の癌である前立腺がんの腫瘍マーカーとして、採血で調べられるのはどれ？", options: ["PSA", "CEA", "AFP", "GPS"], ans: "PSA" },
        { q: "尿道カテーテル（バルーンカテーテル）を留置中の患者。尿バッグの正しい置き場所は？", options: ["常に膀胱より低い位置", "ベッドの上", "膀胱より高い位置", "頭の上"], ans: "常に膀胱より低い位置" },
        { q: "慢性腎臓病(CKD)が末期になり、腎臓の働きがほぼ失われた（尿毒症）場合に導入される治療はどれ？", options: ["血液透析などの腎代替療法", "大量の利尿薬投与", "水分を1日5L飲む", "オメガの祈り"], ans: "血液透析などの腎代替療法" },
        { q: "健常な成人の1日の平均的な尿量はどれくらい？", options: ["1000〜1500mL", "100〜300mL", "3000〜4000mL", "測りきれないほど"], ans: "1000〜1500mL" }
    ];
