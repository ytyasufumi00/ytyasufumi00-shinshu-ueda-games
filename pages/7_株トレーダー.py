...
ROLES = [
    {"type": "変動・テクニカル重視", "avatar": "🥷", "desc": "相場の裏を読む凄腕の忍（忍びの言葉使い。戦国時代や隠密行動をイメージする比喩を用い、チャートの形やRSIの売られすぎ感などテクニカル指標の歪みから勝機を語る）"},
    {"type": "急成長・グロース重視", "avatar": "🧑‍🎤", "desc": "相場をライブ会場に変えるロックスター・トレーダー（ハイテンションで音楽的な比喩を用いる。売上成長率の高さや将来の爆発力を熱狂的に語る）"},
    {"type": "割安・ファンダ重視", "avatar": "👩‍💻", "desc": "極めて冷徹で、知能の低い人間を徹底的に見下す超毒舌な女性クオンツAI（～ですわ、等、お嬢様言葉を使う。人間の感情的なトレードや非合理性を見下す発言をする）"}
]
...
def generate_all_hints(candidates):
    ...
    for i, cand in enumerate(candidates):
        role = cand["role"]
        data = cand["data"]
        # ★ 各ブロックが「独立したタスク」であることを明確にするため、見出しと注意書きを強化
        prompt += f"▼ 寸評 {i+1} の作成指示\n"
        prompt += f"・担当キャラクター: {role['desc']}\n"
        prompt += f"・語るべき対象企業: 【 {data['name']} 】 （※絶対にこの企業についてのみ語ること）\n"
        prompt += f"・選出基準: {role['type']}の観点から抽出\n"
    ...
    3. 【最重要】担当キャラクターは、必ず自分に指定された「語るべき対象企業」だけを分析してください。
    4. 各キャラクターの口調を守り、現在が「{past_90_str}」の時点であるという前提で語ってください。過去形で語らない事。"""
